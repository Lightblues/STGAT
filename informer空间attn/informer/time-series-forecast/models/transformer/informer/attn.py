...
#        values = tf.stack(tf.split(values, H, axis=2), axis=2)
        ...
        queries = tf.stack(tf.split(self.query_norm(self.query_projection(queries)), H, axis=2), axis=2)
        keys = tf.stack(tf.split(self.key_norm(self.key_projection(keys)), H, axis=2), axis=2)
        values = tf.stack(tf.split(self.value_norm(self.value_projection(values)), H, axis=2), axis=2)

        out = self.inner_attention([queries, keys, values], attn_mask=attn_mask)
        out = tf.squeeze(tf.concat(tf.split(out, H, axis=1), axis=3), axis=1)
